%% Script for Fig S7 in the manuscript
% This script plots simulated atmospheric PSD from reduced density cases
%          and observations from aged dust near source regions
% 
...
f_l  = 0.15;
CASES         = {'coarseD_BFT_new', 'coarseD_BFT_rho1000_new', 'coarseD_BFT_rho500_new','coarseD_BFT_rho250_new','coarseD_BFT_rho125_new'}; 
...
for nref = REF_NUMBER{REFGroup}
    ...
    model0_loading_dVdlnD_nref(:,nr) = aa.model2_loading_dVdlnD.*ratio_new_old_function'; %BFT-coarse
    model1_loading_dVdlnD_nref(:,nr) = model1_loading_dVdlnD.*ratio_new_old_function';  %BFT-coarse _rho1000
    model2_loading_dVdlnD_nref(:,nr) = model2_loading_dVdlnD.*ratio_new_old_function';  %BFT-coarse
    model3_loading_dVdlnD_nref(:,nr) = model3_loading_dVdlnD.*ratio_new_old_function';  %BFT-coarse
    model4_loading_dVdlnD_nref(:,nr) = model4_loading_dVdlnD.*ratio_new_old_function';  %BFT-coarse _rho250
    ...
    model0_loading_dVdlnD_nref_avg = mean(model0_loading_dVdlnD_nref,2); %BFT_COARSE
    ...
    model0_loading_dVdlnD_interal_norm2 = f_integral_model_dV(model0_loading_dVdlnD_nref_avg,model_bin_edg,edge_integral);
    ...
    %Normlization 
    model0_loading_dVdlnD_norm2_nref_avg = model0_loading_dVdlnD_nref_avg/model0_loading_dVdlnD_interal_norm2;
 ...
 %% split 0.1-10 bin into 0.2-0.5 and 0.5-1
    ...
    model0_dV = model0_loading_dVdlnD_nref_avg.*model_bin_space_dlnD';
    ...
    [model0_dVdlnD_norm_new]=f_split_first_bin(model0_dV, scheme_name,f_l,edge_integral,model_bin_edge_new);
...
print(p,'-dpng','-r150',sprintf('%s/figS7_agedDust_wSubplots_4panel_lambda_%d',plotpath,f_l*100));
