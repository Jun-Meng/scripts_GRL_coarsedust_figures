...
%            'k-*','linewidth',1.5); hold on %  sim1, for kok11: only show below 20 microns 
    p_cesm_coarse = plot(x3,       model0_loading_dVdlnD_norm,...
           'b->','linewidth',1.5); hold on %  sim2
 
    p_cesm_coarse1000 =plot(x3,       model1_loading_dVdlnD_norm,...
           '-*','color',[113/255,80/255,0/255],'linewidth',1.5); hold on %  rho=1000 
    p_cesm_coarse500 = plot(x3,       model2_loading_dVdlnD_norm,...
           '-*','color',[195/255,120/255,26/255],'linewidth',1.5); hold on %  rho=500
    p_cesm_coarse250 = plot(x3,       model3_loading_dVdlnD_norm,...
           '-*','color',[255/255,189/255,104/255],'linewidth',1.5); hold on %  rho = 250
    p_cesm_coarse125 = plot(x3,       model4_loading_dVdlnD_norm,...
           '-*','color',[255/255,230/255,150/255],'linewidth',1.5); hold on %  rho = 125
...
%         ylim([10^(-4),3]);
        ...
        if REFGroup==1
         ...
         set(the,'location','southeast','FontSize',12,'FontWeight','bold');
         set(the,'color','none');
        end
