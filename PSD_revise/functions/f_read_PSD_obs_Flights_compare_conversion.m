function [obs_d, obs_dVdlnD, ind_lat, ind_lon, ind_lev, date_i,season_i,flight_name]...
          = f_read_PSD_obs_Flights_compare_conversion (inDir,flight_number,YueConvert,instrument)
...
%    season_i = 1 2 3 4 -> (DJF_MAM_JJA_SON)
    ...
    if YueConvert
        % instrument could be 'CDP', 'PCASP' or 'PCASPnCDP'
    	
        %file_obs  = sprintf('%s/insitu/psd_Ryder/psd_corrected/PCASPnCDP/Fennec_b%d_%s_converted.csv', inDir, flight_number,instrument);
        file_obs  = sprintf('%s/insitu/psd_Ryder/psd_corrected/%s/Fennec_b%d_%s_converted.csv', inDir,instrument, flight_number,instrument);
